...
export async function probeLink(link, options = {}) {
	...
	try {
		...
		for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects++) {
			...
			const isDeepLinkPage =
				link.includesSource && checkDeepLinks && checkedUrl.hostname === DEEP_LINK_HOSTNAME;
			const needsSourceBody =
				link.includesSource &&
				(checkParticipation || (checkTokens && link.verificationTokens.size > 0) || isDeepLinkPage);
			let response;
			try {
			...
			if (needsSourceBody) {
				...
				const deepLinkUrl = isDeepLinkPage ? extractDeepLinkUrl(text, checkedUrl.href) : null;
				const deepLinkDetails = deepLinkUrl ? { deepLinkUrl } : {};
